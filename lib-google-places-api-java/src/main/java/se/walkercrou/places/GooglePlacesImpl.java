...
import se.walkercrou.places.model.Place;
import se.walkercrou.places.model.PlaceResponse;
...
public class GooglePlacesImpl implements GooglePlaces {
     ...
     */
    public GooglePlacesImpl(String apiKey, RequestHandler requestHandler) {
        this.apiKey = apiKey;
     ...
     * @param apiKey that has been registered on the Google Developer Console
     */
    public GooglePlacesImpl(String apiKey) {
        this(apiKey, new DefaultRequestHandler());
    }

    /**
     * Creates a new GooglePlaces object using the specified API key and character
     * encoding. Using a character encoding
     * other than UTF-8 is not advised.
     *
     * @param apiKey            that has been registered on the Google Developer Console
     * @param characterEncoding to parse data with
     */
    public GooglePlacesImpl(String apiKey, String characterEncoding) {
    ...
    private static String buildUrl(String method, String params, Param... extraParams) {
        String url =
                String.format(Locale.ENGLISH, "%s%s/json?%s", API_URL, method, params);
     ...
     */
    public static String parse(GooglePlacesImpl client, List<Place> places, String str,
            int limit) {
        ...
        if (statusCode.equals(STATUS_ZERO_RESULTS)) return null;
    ...
    private static void parseResults(GooglePlacesImpl client, List<Place> places,
            JSONArray results, int limit) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            //            // location
            //            JSONObject location = result.getJSONObject(OBJECT_GEOMETRY)
            ...
            //
            //            // see if the place is open, fail-safe if opening_hours is
            // not present
            //            JSONObject hours = result.optJSONObject(OBJECT_HOURS);
            //            boolean hoursDefined = hours != null && hours.has
            // (BOOLEAN_OPENED);
            ...
            //            places.add(place.setClient(client).setPlaceId(placeId)
            // .setLatitude(lat).setLongitude(lon).setIconUrl(iconUrl).setName(name)
    ...
    @Override public boolean isDebugModeEnabled() {
        return debugModeEnabled;
    }

    @Override public void setDebugModeEnabled(boolean debugModeEnabled) {
        this.debugModeEnabled = debugModeEnabled;
    }

    private void debug(String msg) {
        if (debugModeEnabled) System.out.println(msg);
    }
    ...
    @Override public RequestHandler getRequestHandler() {
        return requestHandler;
    }

    @Override public void setRequestHandler(RequestHandler requestHandler) {
        this.requestHandler = requestHandler;
    }

    @Override
    public PlaceResponse getNearbyPlaces(double lat, double lng, double radius, int limit,
            Param... extraParams) {
        try {
            String uri = buildUrl(METHOD_NEARBY_SEARCH,
                    String.format(Locale.ENGLISH, "key=%s&location=%f,%f&radius=%f",
    ...
    @Override public PlaceResponse getNearbyPlaces(double lat, double lng, double radius,
            Param... extraParams) {
        return getNearbyPlaces(lat, lng, radius, DEFAULT_RESULTS, extraParams);
    }

    @Override public PlaceResponse getNearbyPlaces(String nextPageToken) {
        String uri = buildUrl(METHOD_NEARBY_SEARCH,
                String.format("key=%s&pagetoken=%s", apiKey, nextPageToken));
        try {
    ...
    @Override public PlaceResponse getNearbyPlacesRankedByDistance(double lat, double lng,
            int limit, Param... params) {
        try {
            String uri = buildUrl(METHOD_NEARBY_SEARCH,
                    String.format("key=%s&location=%f,%f&rankby=distance", apiKey, lat,
                            lng), params);
            return getPlaces(uri, limit);
        } catch (Exception e) {
            throw new GooglePlacesException(e);
    ...
    @Override public PlaceResponse getNearbyPlacesRankedByDistance(String nextPageToken) {
        String uri = buildUrl(METHOD_NEARBY_SEARCH,
                String.format("key=%s&pagetoken=%s", apiKey, nextPageToken));
        try {
            return getPlacesNextPage(uri);
    ...
    public PlaceResponse getPlacesByQuery(String query, int limit, Param... extraParams) {
        try {
            String uri = buildUrl(METHOD_TEXT_SEARCH,
                    String.format("query=%s&key=%s", query, apiKey), extraParams);
            return getPlaces(uri, limit);
        } catch (Exception e) {
            throw new GooglePlacesException(e);
        }
    ...
    @Override public PlaceResponse getPlacesByQuery(String query, Param... extraParams) {
        return getPlacesByQuery(query, DEFAULT_RESULTS, extraParams);
    }

    @Override public PlaceResponse getPlacesByQuery(String nextPageToken) {
        String uri = buildUrl(METHOD_TEXT_SEARCH,
                String.format("key=%s&pagetoken=%s", apiKey, nextPageToken));
        try {
            return getPlacesNextPage(uri);
    ...
    @Override public PlaceResponse getPlacesByRadar(double lat, double lng, double radius,
            int limit, Param... extraParams) {
        try {
            String uri = buildUrl(METHOD_RADAR_SEARCH,
                    String.format(Locale.ENGLISH, "key=%s&location=%f,%f&radius=%f",
                            apiKey, lat, lng, radius), extraParams);
            return getPlaces(uri, limit);
        } catch (Exception e) {
    ...
    @Override public Place getPlaceById(String placeId, Param... extraParams) {
        try {
            String uri = buildUrl(METHOD_DETAILS,
                    String.format("key=%s&placeid=%s", apiKey, placeId), extraParams);
    ...
    @Override public Place addPlace(PlaceBuilder builder, boolean returnPlace,
            Param... extraParams) {
        try {
    ...
    @Override public void deletePlaceById(String placeId, Param... extraParams) {
        try {
            String uri =
                    buildUrl(METHOD_DELETE, String.format("key=%s", apiKey), extraParams);
    ...
    @Override public void deletePlace(Place place, Param... extraParams) {
    ...
    protected InputStream download(String uri) {
        try {
            ...
            if (in == null) throw new GooglePlacesException(
                    "Could not attain input stream at " + uri);
    ...
    public String getPhotoUrlByReference(String reference, int maxWidth, int maxHeight,
            Param... extraParams) {
        String uri = String.format("%sphoto?photoreference=%s&key=%s", API_URL, reference,
                apiKey);

        List<Param> params = new ArrayList<>(Arrays.asList(extraParams));
        if (maxHeight != -1) {
            params.add(Param.name("maxheight")
                    .value(maxHeight));
        }
        if (maxWidth != -1) {
            params.add(Param.name("maxwidth")
                    .value(maxWidth));
        }
    ...
    private List<Prediction> getPredictions(String input, String method,
            Param... extraParams) {
        try {
    ...
    @Override
    public List<Prediction> getPlacePredictions(String input, int offset, double lat,
            double lng, int radius, Param... extraParams) {
        List<Param> params = new ArrayList<>();
        if (offset != -1) {
            params.add(Param.name("offset")
                    .value(offset));
        }
        if (lat != -1 && lng != -1) {
            params.add(Param.name("location")
                    .value(lat + "," + lng));
        }
        if (radius > 0) {
            params.add(Param.name("radius")
                    .value(radius));
        }
        params.addAll(new ArrayList<>(Arrays.asList(extraParams)));

        return getPredictions(input, METHOD_AUTOCOMPLETE,
                params.toArray(new Param[params.size()]));
    }

    @Override public List<Prediction> getPlacePredictions(String input, int offset,
            Param... extraParams) {
    ...
    @Override
    public List<Prediction> getQueryPredictions(String input, int offset, double lat,
            double lng, int radius, Param... extraParams) {
        List<Param> params = new ArrayList<>();
        if (offset != -1) {
            params.add(Param.name("offset")
        ...
        return getPredictions(input, METHOD_QUERY_AUTOCOMPLETE,
                params.toArray(new Param[params.size()]));
    }

    @Override public List<Prediction> getQueryPredictions(String input, int offset,
            Param... extraParams) {
